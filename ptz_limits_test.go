...
func TestEffectivePTZLimits_FallbackToConstants(t *testing.T) {
	t.Parallel()

	// A daemon with no probed limits should fall back to the package-level
	// lowercase constants in handlers.go (panMin/panMax, tiltMin/tiltMax,
	// zoomMin/zoomMax) so first-boot behaviour (before probe) matches the
	// historical contract.
	//nolint:exhaustruct
	d := &Daemon{}
