...
func (d *Daemon) setGesture(ctx context.Context, enabled bool) error {
	...
	// protocol on the PIXY is not documented and an earlier debug run
...
func (d *Daemon) syncState(ctx context.Context) string {
	...
	// Intentionally skip applying gestureErr / queried gesture state to
	// d.state. The hidInterfaceGesture write protocol used by setGesture
	// (pixyConfig + pixyCommit) does not actually move the EMEET PIXY's
	// gesture-detection flag in firmware - a write-then-readback shows the
	// response bytes unchanged. Treating the queried value as authoritative
	// therefore causes a UI toggle to flip back to its previous position a
	// few seconds after every click. Until the correct write payload is
	// reverse-engineered, the daemon's d.state.Gesture is the user-asserted
	// belief, not the device's, and we leave it alone in this sweep.
	// queryGesture still runs (above) so the readback in setGesture can
	// keep logging the wrote/observed delta for future investigation.
	_ = gestureErr
	_ = gesture
